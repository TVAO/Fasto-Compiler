...
and checkUnaryOp  (ftab : FunTable)
                  (vtab : VarTable)
                  (pos : Position, t : Type, e : UntypedExp)
                : TypedExp =
    let (t', e') = checkExp ftab vtab e
    if t' <> t then
      reportTypeWrong "argument of unary operator" t t' pos
    else
      e'
...
and checkExp  (ftab : FunTable)
    ...
    *)
    | Times (e1, e2, pos) ->
        let (e1_decorator, e2_decorator) = checkBinOp ftab vtab (pos, Int, e1, e2)
        (Int, Times (e1_decorator, e2_decorator, pos))

    | Divide (e1, e2, pos) ->
        let (e1_decorator, e2_decorator) = checkBinOp ftab vtab (pos, Int, e1, e2)
        (Int, Divide (e1_decorator, e2_decorator, pos))

    | And (e1, e2, pos) ->
        let (e1_decorator, e2_decorator) = checkBinOp ftab vtab (pos, Bool, e1, e2)
        (Bool, And (e1_decorator, e2_decorator, pos))

    | Or (e1, e2, pos) ->
        let (e1_decorator, e2_decorator) = checkBinOp ftab vtab (pos, Bool, e1, e2)
        (Bool, Or (e1_decorator, e2_decorator, pos))

    | Not (e, pos) ->
        let e_decorated = checkUnaryOp ftab vtab (pos, Bool, e)
        (Bool, Not (e_decorated, pos))

    | Negate (e, pos) ->
        let e_decorated = checkUnaryOp ftab vtab (pos, Int, e)
        (Int, Negate (e_decorated, pos))
    ...
    *) //  Replicate of Exp<'T> * Exp<'T> * 'T * Position
    ...
    (* TODO project task 2: Hint for `filter(f, arr)`
        ...
        Check that:
            ...
            - the result of filter should have type `[tb]` // TODO: I think this should be [ta]
    *)
    | Filter (f, arr_exp, _, pos) ->
        let (arr_type, arr_exp_dec) = checkExp ftab vtab arr_exp
        let elem_type =
            match arr_type with
              | Array t -> t // arr should be of type [ta]
              | _ -> reportTypeWrongKind "second argument of filter" "array" arr_type pos

        let (f', f_res_type, f_arg_type) =
          match checkFunArg ftab vtab pos f with
            | (f', res, [a1]) -> (f', res, a1) // a1 array_type and res bool type 
            | (_, res, args)  -> reportArityWrong "first argument of filter" 1 (args,res) pos

        if elem_type <> f_arg_type then // f arg should be of type ta 
          reportTypesDifferent "function-argument and array-element types in filter" f_arg_type elem_type pos

        if f_res_type <> Bool then // f result type should be Bool 
          reportTypeWrongKind "function result type in filter" "f" (f_res_type) pos 
    ...
    *)
    | Scan (f, ne_exp, arr_exp, _, pos) -> // _ wildcard is array element type 'T 
        let (ne_type, ne_dec) = checkExp ftab vtab ne_exp // ne = neutral element (e.g. 0 for [int] result)
        let (arr_type, arr_dec) = checkExp ftab vtab arr_exp 
        let elem_type = 
          match arr_type with 
          | Array t -> t 
        ...
        let (f', f_res_type) = 
          match checkFunArg ftab vtab pos f with 
          | (f', res, [a1; a2]) -> 
            if a1 <> a2 // function argument array elements should be of same type as neutral element in e.g. f(ne, arr[i])
              then reportTypesDifferent "argument types of operation in scan" a1 a2 pos 
            if res <> a1 then reportTypesDifferent "argument types of operation in scan" a1 res pos 
            else (f', res)
          | (_, res, args) ->  reportArityWrong "operation in scan" 2 (args,res) pos
          
        if elem_type <> f_res_type // array elements arr[i] should be of same type as function result type f(ne, arr[i])
          then reportTypesDifferent "operation and array-element types in scan" f_res_type elem_type pos

        if ne_type <> f_res_type // function result of f(ne, arr[i]) should be of same type as neutral element (ne)   
          then reportTypesDifferent "operation and start-element types in scan" f_res_type ne_type pos 

        (Array elem_type, Scan(f', ne_dec, arr_dec, elem_type, pos)) // Scan return type array instead of element of arr as reduce
