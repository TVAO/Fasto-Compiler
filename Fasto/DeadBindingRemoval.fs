...
let rec removeDeadBindingsInExp (e : TypedExp) : (bool * DBRtab * TypedExp) =
    match e with
        ...
        | Var (name, pos) ->
            ...
            *)
            // A name cannot contain IO like in: let (nameio, nameuses, e') = removeDeadBindingsInExp e
        ...
        | Index (name, e, t, pos) ->
           ...
           (io, SymTab.combine (SymTab.fromList[(name, ())]) uses, Index (name, e', t, pos))
        ...
        | Let (Dec (name, e, decpos), body, pos) ->
            (* Task 3, Hints for the `Let` case:
                  ...
                  - recursively process the binded expression `e` and
                    join the two results (for `body` and for `e`).
                      -- joining whether the two sub-exps exhibit
                         IO operations: either one or both.
                      -- joining the used-names: semantically union
                         the two symbol tables (`SymTab.combine`),
                         but note that uses of `name` in `body` (only),
                         e.g. `let x = y+1 in x*x`, do _not_ count as
                         `name` being used in the whole let-expression.
            ...
            *)
            ...
            then (eio || bodyio, SymTab.combine euses bodyuses, Let (Dec (name, e', decpos), body', pos))
            else (bodyio, bodyuses, body')
