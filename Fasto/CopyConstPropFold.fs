    ...
    match e with
        ...
        | Var (name, pos) ->
            ...
            *)
            ...
            (* Copy propgation is the process of replacing unwanted bindings, 
               e.g. y = x in z = 3 + y <=> z = 3 + x or y = 3 in z = 3 + y <=> z = 3 + 3 = 6 *)
        ...
        | Index (name, e, t, pos) ->
            ...
            *)
            let e' = copyConstPropFoldExp vtable e // optimize index expression e prehandedly  
            match SymTab.lookup name vtable with // Only copy-propagate variables for indexing 
            ...
            match e' with
                ...
                | Let (Dec (name2, e2, decpos2), body2, pos2) ->
                    ...
                    *)
                    ...
                    (* 1. construct a new let expression as shown above and *)
                    (* 2. optimize it recursively *)

                    copyConstPropFoldExp vtable (Let (Dec (name2, e2, decpos),
                                                      Let (Dec (name, body2, decpos2), body, pos2),
                                                      pos))
        ...
        | Times (e1, e2, pos) ->
            ...
            *)
            ...
            (* Constant folding/propagation is the process of 
               identifying and evaluating constant expressions at compile time instead of runtime *)
