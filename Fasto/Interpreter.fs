...
let rec buildFtab (fdecs: UntypedFunDec list) : FunTable =
    ...
    | [] ->
        ...
        let fdec_chr =
            FunDec("chr", Char, [ Param("n", Int) ], Constant(CharVal ch, p), p)

        let fdec_ord =
            FunDec("ord", Int, [ Param("c", Char) ], Constant(IntVal 1, p), p)

        SymTab.fromList [ ("chr", fdec_chr)
                          ("ord", fdec_ord) ]
    | (fdcl :: fs) ->
...
let rec typeMatch (tpval: Type * Value) : bool =
    ...
    | (Array t, ArrayVal (vals, tp)) ->
        (t = tp)
        && (List.map (fun value -> typeMatch (t, value)) vals
            |> List.fold (&&) true)
    | (_, _) -> false


let invalidOperand (str: string) (tp: Type) (e: Value) (pos: Position) =
    let msg =
        str
        + "; expected "
        + (ppType tp)
...
let invalidOperands (str: string) (tps: (Type * Type) list) (e0: Value) (e1: Value) (pos: Position) =
    let types =
        List.map (fun (tp0, tp1) -> (ppType tp0) + " and " + (ppType tp1)) tps

    let msg =
        str
        + "; expected "
        + (String.concat ", or " types)
        + ", got "
        + (ppType (valueType e0))
        + " ("
        + (ppVal 0 e0)
...
let rec bindParams =
    ...
    | ([], a, fid, pd, pc) ->
        raise (
            MyError(
                "Number of formal and actual params do not match in call to "
                + fid,
                pc
            )
        )
    ...
    | (Param (faid, fatp) :: fargs, a :: aargs, fid, pd, pc) ->
        ...
        if (typeMatch (fatp, a)) then
            ...
            raise (
                MyError(
                    "Actual and formal argument type do not match"
                    + " in function: "
                    + fid
                    + "; formal argument: "
                    + faid
...
let rec evalExp (e: UntypedExp, vtab: VarTable, ftab: FunTable) : Value =
    ...
    | ArrayLit (l, t, pos) ->
        let els =
            (List.map (fun x -> evalExp (x, vtab, ftab)) l)
    ...
    | StringLit (s, pos) ->
        let exps =
            List.map (fun c -> CharVal c) (Seq.toList s)
  ...
  *)
    ...
    | Divide (e1, e2, pos) ->
        ...
        match (res1, res2) with
        | (IntVal n1, IntVal n2) when n2 <> 0 -> IntVal(n1 / n2)
        | (IntVal n1, IntVal n2) when n2 = 0 -> invalidOperand "Division by zero: " Int res2 pos
    ...
    | Equal (e1, e2, pos) ->
        ...
        | (_, _) ->
            invalidOperands
                "Invalid equality operand types"
                [ (Int, Int)
                  (Bool, Bool)
                  (Char, Char) ]
                r1
    ...
    | Less (e1, e2, pos) ->
        ...
        | (_, _) ->
            invalidOperands
                "Invalid less-than operand types"
                [ (Int, Int)
                  (Bool, Bool)
                  (Char, Char) ]
                r1
    ...
    | Apply ("length", [ arg ], pos) ->
        ...
        | otherwise ->
            raise (
                MyError(
                    "Call length function on non-array arg"
                    + ppVal 0 evarg,
                    pos
                )
            )
    | Apply ("length", args, pos) ->
        let msg =
            sprintf "Call to length function expects exactly one arg, given: %i" (List.length args)

        raise (MyError(msg, pos))
    (* general case of function application *)
    | Apply (fid, args, pos) ->
        let evargs =
            List.map (fun e -> evalExp (e, vtab, ftab)) args
    ...
    | Index (id, e, tp, pos) ->
        ...
        | (Some (ArrayVal (lst, tp)), IntVal ind) ->
            ...
            if (len > ind && ind >= 0) then
                lst.Item(ind)
            else
                let msg =
                    sprintf "Array index out of bounds! Array length: %i, index: %i" len ind
    ...
    | Iota (e, pos) ->
        ...
        | IntVal size ->
            if size >= 0 then
                ...
                let msg =
                    sprintf "Error: In iota call, size is negative: %i" size
    ...
    | Map (farg, arrexp, _, _, pos) ->
        ...
        | ArrayVal (lst, tp1) ->
            let mlst =
                List.map (fun x -> evalFunArg (farg, vtab, ftab, pos, [ x ])) lst

            ArrayVal(mlst, farg_ret_type)
        | otherwise ->
            raise (
                MyError(
                    "Second argument of map is not an array: "
                    + ppVal 0 arr,
                    pos
                )
            )
    | Reduce (farg, ne, arrexp, tp, pos) ->
        ...
        | otherwise ->
            raise (
                MyError(
                    "Third argument of reduce is not an array: "
                    + ppVal 0 arr,
                    pos
                )
            )
  ...
  *) 
    | Replicate (e1, e2, t, pos) ->
        ...
        match n with
        | IntVal n when n >= 0  -> ArrayVal (List.replicate n a, valueType a) // type is Int, Bool, Char or Array
        | _ -> raise (MyError("First argument of replicate is not a positive number: " + ppVal 0 n, pos))
  ...
  *)
    | Filter (farg, arrexp, _, pos) ->
        ...
        if farg_ret_type <> Bool then
          let msg = sprintf "Return type of function argument is not bool: %A "
          raise (MyError(msg <| farg_ret_type, pos))
              ...
              let mlst =
                List.filter (fun x ->
                               match (evalFunArg (farg, vtab, ftab, pos, [ x ])) with
                                  | BoolVal v -> v
                                  | _ -> raise(MyError("Return type not boolean...", pos)) // Checked already
                            ) lst
              ArrayVal(mlst, t)
          | _ -> raise (MyError("Second argument of map is not an array: " + ppVal 0 arr, pos))
          
    (* TODO project task 2: `scan(f, ne, arr)`
     Implementation similar to reduce, except that it produces an array
     of the same type and length to the input array `arr`.
     Example: scan(f = +, ne = 0, arr = [1, 2, 3]) = [0+1, 0+1+2, 0+1+2+3] = [1,3,6]
  *)
    | Scan (farg, neexp, arrexp, elem_type, pos) -> 
        let arr = evalExp (arrexp, vtab, ftab) // array 
        let f_ret_type = rtpFunArg farg ftab pos // Checked to be bool in type check
        let ne = evalExp (neexp, vtab, ftab) // neutral element 
        match arr with 
            | ArrayVal (lst, _) -> // wildcard is array elem type
                let mlst = List.scan (fun acc x -> evalFunArg (farg, vtab, ftab, pos, [acc; x])) ne lst |> List.tail // skip ne in mlst[0]
                ArrayVal(mlst, f_ret_type)
            | _ -> raise ( MyError("Third argument of scan is not an array: " + ppVal 0 arr, pos))
                ...
                | otherwise ->
                    raise (
                        MyError(
                            "Write argument "
                            + ppVal 0 v
                            + " should have been evaluated to string",
                            p
                        )
                    )
...
and callFunWithVtable
    ...
    | "ord" ->
        ...
        | otherwise ->
            raise (
                MyError(
                    "Argument of \"ord\" does not evaluate to Char: "
                    + (String.concat "" (List.map (ppVal 0) aargs)),
                    pcall
                )
            )
    | "chr" ->
        ...
        | otherwise ->
            raise (
                MyError(
                    "Argument of \"chr\" does not evaluate to Num: "
                    + (String.concat "" (List.map (ppVal 0) aargs)),
                    pcall
                )
            )
    ...
    | _ ->
        let vtab' =
            SymTab.combine (bindParams (fargs, aargs, fid, pdcl, pcall)) vtab
        ...
        if typeMatch (rtp, res) then
            ...
            raise (
                MyError(
                    "Result type does not match the return type"
                    + " in function: "
                    + fid
                    + " return type: "
                    + ppType (rtp)
