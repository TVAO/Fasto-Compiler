...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  ...
  | Divide (e1, e2, (line, _)) ->
    ...
    let code1 = compileExp e1 vtable t1
    let code2 = compileExp e2 vtable t2 
    let safe_label = newLab "safe_lab"
    let checkdivzero = [ Mips.BNE (t2, RZ, safe_label) // if t2 is not 0, then jump to safe label, otherwise continue 
                       ; Mips.LI (RN5, line) // load error line into register 5 
                       ; Mips.LA (RN6, "_Msg_DivZero_") // load address of div zero error message into register 6 
                       ; Mips.J "_RuntimeError_" // jump to runtime error code 
                       ; Mips.LABEL (safe_label) ]
    code2 @ checkdivzero @ code1 @ [Mips.DIV (place,t1,t2)]

  | Not (e, pos) ->
       // read value of e into register 'place'
       // then negate using XORI
       let code = compileExp e vtable place
       code @ [ Mips.XORI (place, place, 1) ]

  | Negate (e, pos) ->
      let code = compileExp e vtable place
      code @ [ Mips.SUB (place, RZ, place) ]
  ...
  *)
  | And (c1, c2, pos) -> 
      let t1 = newReg "and_L"
      let t2 = newReg "and_R"
      let code1 = compileExp c1 vtable t1
      let code2 = compileExp c2 vtable t2
      let falseLabel = newLab "falseLabel"
      let shortcircuit = [ Mips.LI (place, 0) ; Mips.BEQ (t1, RZ, falseLabel) ] // if cond1 is 0 then short-circuit by jumping over cond2 code 
      code1 @ shortcircuit @ code2 @ [Mips.AND (place, t1, t2); Mips.LABEL falseLabel]
  ...
  | Or (c1, c2, pos) -> 
      ...
      let shortcircuit = [ Mips.LI (place, 1) ; Mips.BNE (t1, RZ, trueLabel) ] // if cond1 is 1 then short-circuit by jumping over cond2 code
      code1 @ shortcircuit @ code2 @ [Mips.OR (place, t1, t2); Mips.LABEL trueLabel]
  ...
  *)
  | Replicate (n_exp, a_exp, tp, (line,_)) ->
      let size_reg = newReg "size_reg" (* size_reg is now the integer n. *)
      let a_reg = newReg "a_reg" (* a_reg is the element a to be replicated. *)
      let addr_reg = newReg "addr_reg" (* addr_reg is now the position of the first array element. *)
      let i_reg = newReg "i_reg" (* index of array *)
      let tmp_reg = newReg "tmp_reg" (* tmp_reg holds temporary result *)

      let safe_lab = newLab "safe_lab" (* label for safe code where N >= 0 *)
      let loop_beg = newLab "loop_beg" (* beginning of loop label*)
      let loop_end = newLab "loop_end" (* end of loop label *)

      let n_code = compileExp n_exp vtable size_reg
      let a_code = compileExp a_exp vtable a_reg
      let elem_size = getElemSize tp

      (* Check that array size N >= 0: if N >= 0 then jumpto safe_lab else jumpto "_IllegalArrSizeError_"; safe_lab: ... *)
      let checksize = [ Mips.BGEZ (size_reg, safe_lab) // jump to safe label if n >= 0 
                      ; Mips.LI (RN5, line)
                      ; Mips.LA (RN6, "_Msg_IllegalArraySize_")
                      ; Mips.J "_RuntimeError_"
                      ; Mips.LABEL (safe_lab)
      ...
      (* Run a loop.  Keep jumping back to loop_beg until i_reg < size_reg, then jump to loop_end. Replicate is always 'arr[i] = element'. *)      
      let loop_header = [ Mips.LABEL (loop_beg)
                        ; Mips.SUB (tmp_reg, i_reg, size_reg)
                        ; Mips.BGEZ (tmp_reg, loop_end) ]

      let loop_replicate = [ mipsStore elem_size (a_reg, addr_reg, 0) ]

      let loop_footer = [ Mips.ADDI (addr_reg, addr_reg, elemSizeToInt elem_size) // Increment by type-specific number of bytes (e.g. 4 for int but 1 for bool)
                        ; Mips.ADDI (i_reg, i_reg, 1)
                        ; Mips.J loop_beg
                        ; Mips.LABEL loop_end ]
      n_code
       @ checksize
       @ a_code
       @ dynalloc (size_reg, place, tp) // dynamically allocate array based on type of elements 
       @ init_regs
       @ loop_header
       @ loop_replicate
       @ loop_footer
  ...
  *)
  | Filter (farg, arr_exp, elem_type, pos) ->
      ...
      let arr_reg  = newReg "arr_reg" (* address of array *)
      let elem_reg = newReg "elem_reg" (* address of single element *)
      let res_reg = newReg "res_reg" (* result *)

      let addr_reg = newReg "addr_reg" (* address of element in new array *)
      let i_reg = newReg "i_reg" (* index register of input array *)
      let j_reg = newReg "j_reg" (* NEW: index of output array that may differ in size from input array *)

      let loop_beg = newLab "loop_beg"
      let loop_end = newLab "loop_end"
      let tmp_reg = newReg "tmp_reg"
      let false_label = newLab "false_label" // jump to label if condition is false on current element  


      let arr_code = compileExp arr_exp vtable arr_reg
      let get_size = [ Mips.LW (size_reg, arr_reg, 0) ]

      let init_regs = [ Mips.ADDI(addr_reg, place, 4) // increment input array by 4 to skip n (size) and move to first element 
                      ; Mips.MOVE(i_reg, RZ) // set i = 0 
                      ; Mips.MOVE(j_reg, RZ) // set j = 0 
                      ; Mips.ADDI(elem_reg, arr_reg, 4) ]
      ...
      let elem_size = getElemSize elem_type
      let loop_filter0 = mipsLoad elem_size (res_reg, elem_reg, 0) :: applyFunArg(farg, [res_reg], vtable, res_reg, pos)
      let loop_filter1 = [ Mips.BEQ(res_reg, RZ, false_label) // if condition 0 jump to false label 
                         ; mipsLoad elem_size (res_reg, elem_reg, 0)
                         ; mipsStore elem_size (res_reg, addr_reg, 0)]
      
      let loop_footer = [ Mips.ADDI(addr_reg, addr_reg, elemSizeToInt elem_size)
                        ; Mips.ADDI(j_reg, j_reg, 1) // only increment j in output array if true condition 
                        ; Mips.LABEL false_label // only increment i in input array if false condition (not j for output array)
                        ; Mips.ADDI(elem_reg, elem_reg, elemSizeToInt elem_size)
                        ; Mips.ADDI(i_reg, i_reg, 1) // increment i by 1 
                        ; Mips.J loop_beg // jump back to beginning of loop 
                        ; Mips.LABEL loop_end // end of loop 
                        ; Mips.SW (j_reg, place, 0) ] // NEW: reset j to start of output array 
                
      arr_code 
        @ get_size
        @ dynalloc (size_reg, place, elem_type) // allocate array of same type as input array 
        @ init_regs
        @ loop_header
        @ loop_filter0
        @ loop_filter1
  ...
  *)
  | Scan (binop, acc_exp, arr_exp, tp, pos) ->
      ...
      let loop_end = newLab "loop_end"
      let addr_reg = newReg "addr_reg" (* address of element in new array *)
      let acc_reg  = newReg "acc_reg" // NEW: added register for accumulator 

      let arr_code = compileExp arr_exp vtable arr_reg 
      let header1 = [ Mips.LW(size_reg, arr_reg, 0) ] // load array size n stored in arr[0] as int of 4 bytes  

      let init_regs = [ Mips.ADDI (addr_reg, place, 4) ] // NEW: increment new array by 4 bytes to skip first element n (size) 

      (* Compile initial value into place (will be updated below) *)
      let acc_code = compileExp acc_exp vtable acc_reg // place (destination) replaced by acc_reg 

      (* Set arr_reg to address of first element instead. *)
      (* Set i_reg to 0. While i < size_reg, loop. *)
      let loop_code = 
              [ Mips.ADDI(arr_reg, arr_reg, 4) // increment array by 4 bytes to skip first element n (size of array) in Fasto array layout
              ; Mips.MOVE(i_reg, RZ) // i = 0 where register RZ content is always 0
              ; Mips.LABEL(loop_beg) // beginning of loop 
              ; Mips.SUB(tmp_reg, i_reg, size_reg) // tmp := i - n 
              ; Mips.BGEZ(tmp_reg, loop_end) // if tmp = 0: end loop <=> i >= n, e.g. if n = i then i < n no longer holds and i - n = 0
              ]
      
      (* Load arr[i] into tmp_reg *)
      // Use mipsLoad and getElemSize that dynamically uses lw (load word) with 4 bytes for e.g. integers and lb for e.g. booleans with 1 byte 
      let elem_size = getElemSize tp 
      let load_code = 
        [ 
          mipsLoad elem_size (tmp_reg, arr_reg, 0) // load size of array into temp and store in local elem_size binding 
        ; Mips.ADDI(arr_reg, arr_reg, elemSizeToInt elem_size) // increment to next element with offset based on type (e.g. 1 for bool)
        ]
      
      // Code used to store temporary last computed result in accumulator 
      let store_code = 
        [
           mipsStore elem_size (acc_reg, addr_reg, 0) // store current accumulator result in new array element result
        ;  Mips.ADDI(addr_reg, addr_reg, elemSizeToInt elem_size) // increment address of element in new array to point to next result  
        ]
      ...
      arr_code
        @ header1
        @ dynalloc (size_reg, place, tp) // allocate n bytes for array of type tp at destination
        @ init_regs
        @ acc_code 
        @ loop_code 
        @ load_code 
        ...
        @ store_code 
        @ 
         [ Mips.ADDI(i_reg, i_reg, 1) // increment i 
         ; Mips.J loop_beg // jump back to loop condition 
         ; Mips.LABEL loop_end // end loop 
         ]
